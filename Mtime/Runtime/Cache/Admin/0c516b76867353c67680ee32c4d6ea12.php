<?php if (!defined('THINK_PATH')) exit();?><html>
	...
	<div class="container">
		<div class="panel panel-info">
			<div class="panel-heading">
				<p style="font-size:16px;" style="float:left;">会员评论回复</p>
				<a class="btn btn-primary" id="btn_fh" style="float:right;margin-top:-30px;margin-right:10px;">返回</a>
			</div>
			<div class="panel-body">
				<p><span style="color:#31708F;"><?php echo ($comtitle['comment_title']); ?></span></p>
			</div>
			<table class="table table-hover table-striped table-bordered">
				...
				<?php if(is_array($recoms)): foreach($recoms as $key=>$recom): ?><tr>
						...
						<td>
							<a href="/mtime/index.php/Admin/Comment/lookrecommentreplay/id/<?php echo ($recom['recomment_id']); ?>">查看评论</a>
						</td>
						<td>
	...
	<script>
		...
		$(".btn_check").click(function(){
			...
			status = $(this).parent().next().next().next().children().text();
					...
					if(d=='1'){
						switch(parseInt(status)){
							case 0:
								...
								obj.parent().next().next().next().children().text("0");
								break;
							case 1:
								obj.parent().prev().text("审核通过").css({"color":"green"});
								obj.parent().next().next().next().children().text("1");
								break;
						}
					}
				}
