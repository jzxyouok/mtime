	...
	*/
	class Page {
		...
		function fpage(){
			...
			if(count($arr) < 1)
				$arr = array(0,1,2,3,4,5,6,7);    //用于更改页面分页条显示的结构
		...
		private function goPage(){
    			if($this->pageNum > 1) {
				return '&nbsp;<input style="margin-top:9px;text-align:center;width:30px;height:17px !important;height:25px;line-height:10px;border:1px solid #ccc;" type="text" onkeydown="javascript:if(event.keyCode==13){var page=(this.value>'.$this->pageNum.')?'.$this->pageNum.':this.value;location=\''.$this->uri.'page=\'+page+\'\'}" value="'.$this->page.'"><input style="cursor:pointer;width:35px;height:18px;line-height:15px;border:1px solid #ccc;" type="button" value="GO" onclick="javascript:var page=(this.previousSibling.value>'.$this->pageNum.')?'.$this->pageNum.':this.previousSibling.value;location=\''.$this->uri.'page=\'+page+\'\'">&nbsp;';
			}
		}
